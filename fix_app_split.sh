#!/bin/bash
# Move Settings Modal out of App.tsx
cat << 'INNEREOF' > src/components/SettingsModal.tsx
import React, { useRef } from 'react';
import { X, Moon, Sun, Camera, FileText } from 'lucide-react';
import { AppThemeType, ButtonStyleType } from '../types';
                  ...
                  {appThemes.map(tPreset => (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm leading-tight">{language === 'kh' ? tPreset.nameKh : tPreset.name}</span>
                      </div>
                    </button>
                  ))}
                </div>
